...
train_loader = DataLoader(dataset=train_dataset, batch_size=BATCH_SIZE, shuffle=True, collate_fn=notes_collate_fn,
						  num_workers=NUM_WORKERS)
valid_loader = DataLoader(dataset=valid_dataset, batch_size=BATCH_SIZE, shuffle=False, collate_fn=notes_collate_fn,
						  num_workers=NUM_WORKERS)
# batch_size for the test set should be 1 to avoid sorting each mini-batch which breaks the connection with patient IDs
test_loader = DataLoader(dataset=test_dataset, batch_size=1, shuffle=False, collate_fn=notes_collate_fn,
						 num_workers=NUM_WORKERS)
...
for epoch in range(NUM_EPOCHS):
	...
	if is_best:
		...
		torch.save(model, os.path.join(PATH_OUTPUT, "MyVariableRNN.pth"),
				   ...
				   # TO AVOID PROBLEMS WITH LOADING THE PTH FILE ON GRADESCOPE? APPARENTLY IT'S A PYTORCH VERSION PROBLEM?
				   )
